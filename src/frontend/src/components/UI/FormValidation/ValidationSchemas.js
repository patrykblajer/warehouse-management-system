...
export function getProductFormValidationSchema() {
    const availableRules = {
        ...
        maxTwoDecimalPlaces: {
            regex: /^\d+(\.\d{1,2})?$/, message: 'Dopuszczalna wartość do drugiego miejsca po przecinku.',
        },
        maxNumber: {
            max: 8, message: 'Podana wartość jest nieprawidłowa.',
        },
    }
    return Yup.object({
        name: Yup.string()
        ...
        category: Yup.object().required(availableRules.required.message),
        unit: Yup.object().required(availableRules.required.message),
